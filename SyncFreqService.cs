...
{
    public class SyncFreqService
    {
        private const string responseMsgCode = "PIVS0051";

        
        private static string err = "";

        #region 同步频次
        public static string saveFreq(string jsonStr)
        {
            string key = "fcode";
            string val = "无法读取属性";
            int info = 0;
            ...
            try
            {
                ...
                dt.Rows.Add(new object[] { row["fcode"].ToString(), row["fname"].ToString(), row["timestring"].ToString(), row["idupdate"].ToString() });
                }
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    {
                        case "insert":
                            sql.Append("insert into [DFreq]([FreqCode],[FreqName],[UseTime],[flag]) values(");
                            sql.Append("'" + dt.Rows[i]["fcode"].ToString() + "'");
                            sql.Append(",'" + dt.Rows[i]["fname"].ToString() + "'");
                            sql.Append(",'" + dt.Rows[i]["timestring"].ToString() + "'");
                            sql.Append(",0");
                            sql.Append(")");
                            break;
                        case "update":
                            sql.Append("update [DFreq] set");
                            sql.Append(" [DFreq].[FreqName] = '" + dt.Rows[i]["fname"].ToString() + "'");
                            sql.Append(",[DFreq].[UseTime] = '" + dt.Rows[i]["timestring"].ToString() + "'");
                            sql.Append(",[DFreq].[flag] = 0");
                            sql.Append(" where [DFreq].[FreqCode] = '" + dt.Rows[i]["fcode"].ToString() + "'");
                            break;
                        case "delete":
                            sql.Append("delete from [DFreq]");
                            sql.Append(" where [DFreq].[FreqCode] = '" + dt.Rows[i]["fcode"].ToString() + "'");
                            break;
                        default:
                            info = 3;
                ...
                if (count == dt.Rows.Count)
                {
                    ...
                    for (int j = 0; j < dt.Rows.Count; j++)
                    {
                        ...
                        string updateSql = "update [DFreq] set flag = 1 where [DFreq].[FreqCode] = '" + dt.Rows[j]["fcode"].ToString() + "'";
            ...
            writeContent.writeOutLog("Freq_" + DateTime.Now.ToString("HHmmss"), "HISLog", result);
            return result;
        }
        #endregion
    }
